...
# edit the base variable to point to newly created 'jump' directory
# there should be two sub-directories in the 'jump' directory
# 1) data 2) models     <---- mind the case, its sensitive. ;)
# Please report all the bugsy to jumplabs. the codes are
...
def getNewClass(name="Anonymous", gender="No Mention", age=0, relation_with_family="NA"):
    ...
    id = name + "_" + gender + "_" + str(age) + "_" + relation_with_family + "_" + str(random.randint(1, 10000)) + str(random.randint(500, 50000))
    ...
    ################   Open camera stream #################
    if training_camera_url != "":
        print("accessing camera at url: ", training_camera_url)
        #video_capture = cv2.VideoCapture('rtsp://admin:admin@192.168.1.152:554')  # cp plus camera
        video_capture = cv2.VideoCapture(training_camera_url)  # cp plus camera
...
if __name__ == "__main__":
    ...
    while res == 'y':
        ...
        name = input("Name of person: ") or "Anonymous"
        gender = input("gender of person: ") or "No Mention"
        age = int(input("age of person: ") or 0 )
        relation_with_family = input("relation_with_family: ") or "NA"

        #print("you entered values as ", name , gender, age,relation_with_family)
        getNewClass(name, gender, age, relation_with_family)
